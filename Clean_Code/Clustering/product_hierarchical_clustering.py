"""
Product hierarchical clustering script for VectorDB.
Uses the hierarchical clustering pipeline optimized for product data.
"""
...
from Clustering.hierarchical_pipeline import HierarchicalClusteringPipeline
...
def main():
    ...
    for level, level_results in results["levels"].items():
        ...
        logger.info(f"Level {level}: {num_clusters} clusters, {points_assigned}/{len(descriptions)} points assigned ({noise_percentage:.2f}% noise)")
